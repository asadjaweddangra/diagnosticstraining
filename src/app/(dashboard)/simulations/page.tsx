    ...
    <div className="space-y-6">
      <div className="rounded-xl bg-white border border-gray-200 shadow-sm p-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold text-gray-900">Simulator Drills & Case Assist</h1>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {drills.map((d, idx) => (
          <Link
            key={`${d.title}-${idx}`}
            href={d.href}
            className="group flex h-full flex-col justify-between rounded-xl bg-white border border-gray-200 shadow-sm p-4 transition hover:shadow-md hover:-translate-y-0.5"
          >
            ...
            <div className="mt-2 space-y-2">
              ...
              <p className="text-xs text-gray-600 line-clamp-3">{d.scenario}</p>
              <ul className="list-disc space-y-1 pl-5 text-xs text-gray-600">
                {d.steps.slice(0, 3).map((s, i) => (
                  <li key={i}>{s}</li>
                ))}
              </ul>
              {d.takeaway ? <p className="text-xs font-semibold text-primary-700">{d.takeaway}</p> : null}
            </div>
            <div className="mt-3 inline-flex items-center gap-2 text-xs font-semibold text-primary-600">
